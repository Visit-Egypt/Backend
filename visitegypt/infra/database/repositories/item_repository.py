...
from visitegypt.core.items.entities.item import ItemInDB, ItemsPageResponse, ItemBase, ItemUpdate
...
async def get_filtered_items(page_num: int, limit: int, filters: Dict) -> ItemsPageResponse:
    try:
        indcies = calculate_start_index(limit, page_num)
        start_index : int = indcies[0]
        cursor = db.client[DATABASE_NAME][items_collection_name].find(filters).skip(start_index).limit(limit)
...
async def create_item(item_to_create: ItemBase) -> ItemInDB:
    try:
        row = await db.client[DATABASE_NAME][items_collection_name].insert_one(item_to_create.dict())
        if row.inserted_id:
            new_inserted_item = await get_filtered_items(page_num=1, limit=1, filters={"_id": row.inserted_id})
...
async def update_item(item_to_update: ItemUpdate, item_id: str) -> ItemInDB:
    try:
        result = await db.client[DATABASE_NAME][items_collection_name].find_one_and_update({"_id": ObjectId(item_id)}, {'$set': {k: v for k, v in item_to_update.dict().items() if v}}, return_document=ReturnDocument.AFTER)
        if result: return ItemInDB.from_mongo(result)
...
async def delete_item(item_id: str) -> bool:
    try:
        res = await db.client[DATABASE_NAME][items_collection_name].delete_one({"_id": ObjectId(item_id)})
