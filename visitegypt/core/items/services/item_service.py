...
async def get_filtered_items(
    repo: ItemRepo, page_num: int = 1, limit: int = 15, filters: Dict = None
) -> ItemsPageResponse:
    try:
...
async def create_item(repo: ItemRepo, item_to_create: ItemBase) -> ItemInDB:
    try:
        c_i = await get_filtered_items(
            repo, page_num=1, limit=1, filters={"title": item_to_create.title}
        )
        if c_i.items[0]:
            raise ItemAlreadyExists
    except ItemAlreadyExists as iee:
        raise iee
...
async def update_item_by_id(
    repo: ItemRepo, item_to_update: ItemUpdate, item_id: str
) -> Optional[ItemInDB]:
    try:
