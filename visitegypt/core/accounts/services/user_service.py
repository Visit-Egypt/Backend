...
async def register(repo: UserRepo, new_user: UserCreate) -> UserResponse:
    ...
    try:
        ...
        if isinstance(e, UserNotFoundError): pass
        else: raise e
...
async def update_user_by_id(repo: UserRepo,updated_user:UserUpdate, user_id: str) -> Optional[UserResponse]:
    try:
        ...
        raise e
async def update_user_role(repo: UserRepo,updated_user:str, user_id: str) -> Optional[UserResponse]:
    try:
