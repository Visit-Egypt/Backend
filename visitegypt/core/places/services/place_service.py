from visitegypt.core.places.entities.place import PlacesPageResponse, CreatePlace,PlaceInDB, UpdatePlace, review, PlaceBase
from visitegypt.core.places.protocols.place_repo import PlaceRepo
from typing import Dict, Optional
from visitegypt.core.errors.place_error import *

async def get_all_places_paged(repo: PlaceRepo, page_num: int = 1, limit: int = 15) -> PlacesPageResponse:
    try:
...
async def create_place(repo: PlaceRepo, new_place: PlaceBase) -> PlaceInDB:
    try:
        ...
        if place: raise PlaceAlreadyExists
    except PlaceAlreadyExists as iee: raise iee
    except PlaceNotFoundError: pass
...
async def update_place(repo: PlaceRepo,place_to_update:UpdatePlace, place_id: str) -> Optional[PlaceInDB]:
    try:
...
async def add_review(repo: PlaceRepo,place_id:str, new_reviw:review):
    try:
        ...
        if place_reviews: return place_reviews
    except PlaceNotFoundError as ue:
        raise ue
    except Exception as e:
        raise e
